...
# cd $HOME/shared
# python3 SnailTrail_L2Norm_OptimizeKappa.py
###############################################################################
# Solves for the optimal value of Kappa used in the 2D Snail-Trail PDE System
# described in Martinson et al. (2020) for a Unit Square domain with No-Flux
# BCs.
# The value of Kappa minimizes ABM data specified as CSV files. ABM Data at
...
## Set up c(x,y,t). Since this is assumed to be at steady state, it is thus
...
def res_fun_L2norm(kappa, ABM_TCdata, ABM_ECdata, mesh, x, y, V, A, Q, n_init_cond, e_init_cond, c):
    ...
    output1 = File("Tip_Cells_Optimized_Kappa.pvd") # n(x,y,t)
    output2 = File("Stalk_Cells_Optimized_Kappa.pvd") # e(x,y,t)
    output3 = File("TAF_Concentration_Optimized_Kappa.pvd") # c(x,y,t)
    ...
    TC_CSVname = 'Tip_Cells_Optimized_Kappa_CSV_Data_t'
    EC_CSVname = 'Tip_Cells_Optimized_Kappa_CSV_Data_t'
    X2 = mesh.coordinates()
...
###########################   Crank-Nicolson Method      ######################
        ...
        if nTimeStep%10==0:
            ...
            output1<<(n_k, float(t))
            output2<<(e_k, float(t))
        # Output to CSV data in increments of 0.2 seconds
        if nTimeStep%200==0:
            (n_k, e_k) = q_prevs.split()
...
kappa0 = 4.5
# Optimize the value of kappa using a nonlinear least squares method:
optimal_kappa = least_squares(res_fun_L2norm, kappa0, bounds = (0.0,np.inf), args = (ABM_TCdata, ABM_ECdata, mesh, x, y, V, A, Q, n_init_cond, e_init_cond, c))
# Print to terminal the value of kappa and the termination conditions
# of the algorithm
print('Optimal value of Kappa is ')
print(optimal_kappa.x)
print(optimal_kappa.message)
# Save the value of kappa, the residuals for that value, and the jacobian
# so that you can compute 95% confidence intervals:
np.savetxt('OptimalKappa_Value.csv', optimal_kappa.x, delimiter=',')
np.savetxt('OptimalKappa_Residuals.csv', optimal_kappa.fun, delimiter=',')
np.savetxt('OptimalKappa_Jacobian.csv', optimal_kappa.jac, delimiter=',')
