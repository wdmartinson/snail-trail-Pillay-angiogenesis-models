...
# cd $HOME/shared
# python3 SnailTrail_1D_L2Norm_Optimize_Beta_e.py
###############################################################################
# Solves for the optimal value of Beta_e used in the 1D Snail-Trail PDE System
# described in Martinson et al. (2020) for a Unit Interval domain with No-Flux
# BCs for the tip cells.
# The value of Beta_e minimizes column averaged ABM data specified as CSV files.
...
# function on a 2D grid. Use linear interpolation to create this function:
...
# Define a function to store the ABM data:
def y_fun(*args, **kwargs):
    ...
    for i in [4,6,8,10,12,14,16,18,20]:
        TC_name = 'Tip_Cell_ColumnAveraged_ABM_Data_t'+str(i)+'.csv'
        EC_name = 'Tip_Cell_ColumnAveraged_ABM_Data_t'+str(i)+'.csv'
        next_TCdata = np.loadtxt(TC_name, delimiter = ',')
        next_TCdata = next_TCdata[:,1]
        next_ECdata = np.loadtxt(EC_name, delimiter = ',')
        next_ECdata = next_ECdata[:,1]
...
# Quadratic Lagrange polynomials on interval element
V = FiniteElement("CG",interval, 2) # Define element
...
# Set up Initial Condition on the domain using the CSV Data:
...
def res_fun_L2norm(Beta_e, ABM_TCdata, ABM_ECdata, mesh, x, V, A, Q, n_init_cond, e_init_cond, c):
    ...
    output1 = File("Tip_Cells_1DSnailTrailPDE_Optimized_Beta_e.pvd") # n(x,y,t)
    output2 = File("Stalk_Cells_1DSnailTrailPDE_Optimized_Beta_e.pvd") # e(x,y,t)
    output3 = File("TAF_Concentration_1DSnailTrailPDE_Optimized_Beta_e.pvd") # c(x,y,t)
    ...
    TC_CSVname = 'Tip_Cells_1DSnailTrailPDE_Optimized_Beta_e_CSV_Data_t'
    EC_CSVname = 'Stalk_Cells_1DSnailTrailPDE_Optimized_Beta_e_CSV_Data_t'
    X2 = mesh.coordinates()
...
optimal_beta_e = least_squares(res_fun_L2norm, beta_e0, bounds = (0.0,160.0), args = (ABM_TCdata, ABM_ECdata, mesh, x, V, A, Q, n_init_cond, e_init_cond, c))
...
np.savetxt('Optimal_Beta_e_1DSnailTrailModel_Value.csv', optimal_beta_e.x, delimiter=',')
np.savetxt('-Optimal_Beta_e_1DSnailTrailModel_Residuals.csv', optimal_beta_e.fun, delimiter=',')
np.savetxt('Optimal_Beta_e_1DSnailTrailModel_Jacobian.csv', optimal_beta_e.jac, delimiter=',')
